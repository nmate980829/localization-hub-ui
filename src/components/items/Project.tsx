...
import { InviteProps, ProjectProps } from './types';
...
export const ProjectItem: React.FC<ProjectProps> = ({ item, remove }) => {
  ...
  return useObserver(() => (
    ...
    >
      <Skeleton isLoaded={loaded} w="100%" rounded={30}>
        <LinkBox as="article">
          <LinkOverlay to={`/projects/${item.id}`} as={Link} />
          <MotionFlex
            direction="row"
            align="center"
